...
import type { EmailLoginPayload, LoginCaptchaResult, LoginPayload, PasswordPayload, ProfilePayload, RegisterPayload, ResetPasswordPayload, Session, UsernameCheckResult } from '@/types'
...
export async function createLoginCaptcha(username: string): Promise<LoginCaptchaResult> {
  const { data } = await api.post('/auth/login-captcha', { username })
  return data
}
