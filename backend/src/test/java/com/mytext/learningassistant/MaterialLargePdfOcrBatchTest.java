...
/**
 * 大 PDF 图片页后台 OCR 分批补齐测试。
 *
 * <p>该测试强制小 PDF 也走大 PDF 快速导入分支，并用固定命令模拟 OCR 输出，
 * 确认初次导入只生成占位页后，后台 OCR 任务可以把占位 chunk/page/textLayer
 * 替换为可检索正文。</p>
 */
@SpringBootTest(properties = {
    ...
    "app.ocr.command-template=cmd /c echo LARGE_PDF_BATCH_OCR_TEXT"
})
@AutoConfigureMockMvc
class MaterialLargePdfOcrBatchTest {
    ...
    @Test
    void largeImagePdfBackfillsOcrTextInBackgroundBatch() throws Exception {
        ...
        String token = registerAndLogin("large_pdf_ocr_user_" + UUID.randomUUID().toString().replace("-", "").substring(0, 10));
        ...
        var uploadResult = mockMvc.perform(multipart("/api/materials")
                ...
                .param("title", "Large Image OCR PDF")
        ...
        mockMvc.perform(get("/api/materials/" + materialId)
            ...
            .andExpect(jsonPath("$.data.ocrStatus").value("READY"))
            .andExpect(jsonPath("$.data.processingProgressPercent").value(100));
        ...
        mockMvc.perform(get("/api/materials/" + materialId + "/chunks")
            ...
            .andExpect(jsonPath("$.data[0].chunkText").value(Matchers.containsString("LARGE_PDF_BATCH_OCR_TEXT")));

        mockMvc.perform(get("/api/materials/" + materialId + "/pages/1/text-layer")
                .header("Authorization", "Bearer " + token))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.data[0].text").value(Matchers.containsString("LARGE_PDF_BATCH_OCR_TEXT")));
    }
