...
interface PersistedChatDraft { /* 持久化到 sessionStorage 的草稿格式 */ }
interface ConversationArchiveItem { /* 对话归档格式 */ }
...
const defaultState: ChatSessionSnapshot = {
  ...
  messages: [], conversationHistory: [], streaming: false, images: [],
}
...
export function resetChatSession(options?: {
  ...
  images?: ChatImagePayload[]; abortActive?: boolean
}) {
...
export function startChatSessionStream(params: {
  ...
  chunkId: string | null; images?: ChatImagePayload[]
}) {
  ...
  state = {
    ...
    images: [], messages: state.messages.concat(userMsg, thinkingMsg),
  ...
  activeController = chatStream(
    ...
    },
    {
      ...
      onChunk: (delta) => {
        ...
        answer += delta  // 累积原始文本
        const cleanText = sanitizeAiText(answer)  // 去掉 Markdown 标记
        state = { ...state, messages: state.messages.map((m) =>
...
function restoreSnapshot(): ChatSessionSnapshot {
  ...
  try {
    ...
    return {
      ...
      mode: draft.mode, materialId: draft.materialId, chunkId: draft.chunkId,
      // 处理中断的流式消息
...
function persistSnapshot() {
  ...
  const draft: PersistedChatDraft = {
    ...
    messages: state.messages, conversationHistory: state.conversationHistory, images: state.images,
  }
...
/** 从归档中读取指定对话 */
function readConversationArchive(conversationId: string) {
  return readConversationArchiveMap()[conversationId] || null
}

/** 读取所有归档对话 */
function readConversationArchiveMap() {
  if (typeof window === 'undefined') return {}
